...
#define USB_VENDOR_ID 0x09c5
#define USB_PRODUCT_ID 0x0588

//#define USB_VENDOR_ID 0x23a9
//#define USB_PRODUCT_ID 0xef18
...
int close_device(libusb_device_handle* &g_usb_handle,struct userDevice &user_device){

    libusb_close(g_usb_handle);

    int rv;
    rv = libusb_release_interface(g_usb_handle,user_device.bInterfaceNumber);
    if(rv <0){
       printf("libusb_release_interface fail:%d \n",rv);
